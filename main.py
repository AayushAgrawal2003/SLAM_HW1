...
def init_particles_freespace(num_particles, occupancy_map):
    ...
    # initialize [x, y, theta] positions in world_frame for all particles
    # Find all free space cells: occupancy probability is low (close to 0)
    # and not unknown (-1)
    free_mask = (occupancy_map >= 0) & (occupancy_map < 0.35)
    free_indices = np.argwhere(free_mask)  # [row, col] = [y_map, x_map]

    # Randomly sample from free cells
    chosen = free_indices[np.random.choice(free_indices.shape[0], size=num_particles, replace=True)]

    # Convert map indices to world coordinates (cm)
    resolution = 10  # cm per cell
    x0_vals = chosen[:, 1:2] * resolution  # col -> x
    y0_vals = chosen[:, 0:1] * resolution  # row -> y
